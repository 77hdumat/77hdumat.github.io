...
이러한 문제를 해결하기 위해 적은 리소스로 큰 효과를 낼 수 있는 (판교 사투리로 ASAP 하게 😅) 의식의 흐름대로 비즈니스 로직을 오염 시키고 있는 캐시를 걷어내고 
지속 가능하고 간편한 캐시 관리 방안을 모색했습니다.
...
| `beforeInvocation`     | `true`일 경우, 메서드 실행 **전**에 캐시 삭제 (메서드 성공 여부와 무관) |
...
export const createDecorator = (
    ...
    const aopSymbol = Symbol('AOP_DECORATOR'); // 고유 심볼
    return applyDecorators(
        // 메타데이터 저장
        (target: object, propertyKey: string | symbol, descriptor: PropertyDescriptor) => {
          // Reflect를 통해 메서드에 부착되고 추후 Discovery 과정에서 식별됩니다.
          return AddMetadata<symbol | string, AopMetadata>(metadataKey, {
            originalFn: descriptor.value, // 원본 메서드 (임시 저장, wrapMethod 참고)
            metadata, // 데코레이터에 전달된 인자
            aopSymbol,
          })(target, propertyKey, descriptor);
        },
    
        // 원본 메서드에 프록시 메서드 덮어쓰기 
...
프록시 함수는 `this[aopSymbol][methodName]` 을 호출하며, **[교체]** 단계에서 wrappedFn을 연결해 두었기 때문에 AOP 로직이 실행됩니다.
